...
#+begin_src R :exports both :session :results output graphics file :file ../img/3_calories.png
...
- Scatterplot with base R ~plot~
  #+begin_src R :results output graphics file :file ../img/3_cereal.png
    library(MASS)
...
- Scatterplot with ~grid~ package:
  #+begin_src R :results output graphics file :file ../img/3_grid.png
    library(MASS)
    x <- UScereal$sugars
    y <- UScereal$calories
    library(grid)
...
- [[https://cran.r-project.org/web/packages/lattice/lattice.pdf][lattice]] has different default options for plot customization and
  ...
  #+begin_src R :results output graphics file :file ../img/lattice.png
