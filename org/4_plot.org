...
* NEXT Customization with vectors
...
- Our final R code block:
  #+begin_src R :results graphics file :file ../img/4_cereals5.png
    plot(x,y,
         xlab="Grams of sugar per serving",
         ylab="Calories per serving",
         type = "n")
...
- Example:
  #+begin_src R :results graphics file :file ../img/4_las.png
    par(las=0, mfrow=c(2,2))
...
- The default axes are now suppressed and you need to set:
  ...
  3) ~side = 3~ : creates upper x-axis above theplot
...
- Box plot without axes: The ~varwidth~ parameter creates a boxplot of
  ...
  #+begin_src R :results graphics file :file ../img/4_axis1.png
    boxplot(
      sugars ~ shelf,
      data = UScereal,
...
- Box plot with axis specified using ~axis~: 
  #+begin_src R :results graphics file :file ../img/4_axis2.png
    boxplot(
      sugars ~ shelf,
      data = UScereal,
      axes = FALSE,  # this removes the default axes
