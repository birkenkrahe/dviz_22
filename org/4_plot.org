...
- Several parameters come in groups:
  ...
  2) The ~col~ family specifies colors for points, lines, text[fn:2]. To
...
- The code block is growing, so let's look at the ~text~ addition alone:
  ...
  2) Let's pick the expression ~rownames(UScereal)[index]~ apart[fn:1]:
...
[fn:1] The need to deconstruct, as it were, complex functional
