...
- All these commands are contained in the function
  ~ConvertAutoMpgRecords~. An R file with the function is here:
  https://pastebin.com/C4bmppbm.
  1) Download the file from this URL as ~ConvertAutoMpgRecords.R~
  2) Run the file using ~shell~ from within R

  It is saved as a file with that name. You can
  ...
  #+RESULTS:
  #+begin_example
  trying URL 'https://drive.google.com/file/d/1-2V_a0-GBkDg95MIHIjuhlMQ77ghK9df/view?usp=share_link'
  downloaded 72 KB
...
- Amazingly, R's download function can recover the GDrive file!
